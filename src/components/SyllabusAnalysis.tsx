...
export default function SyllabusAnalysis({ topics, mastery, onStartAssessment }: SyllabusAnalysisProps) {
    const totalTopics = topics.length;
    if (totalTopics === 0) return null;

    const needsAssessment = topics.filter(t => !mastery.find(m => m.topicId === t.id));
                ...
                <div className="bg-primary/5 border border-primary/20 p-6 rounded-3xl">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-sm font-bold text-primary uppercase tracking-widest flex items-center gap-2">
                            ...
                            Assessment Required ({needsAssessment.length})
                        </h3>
                    </div>
                    <div className="flex flex-wrap gap-2">
                        {needsAssessment.map(topic => (
                            <button
                                key={topic.id}
                                onClick={() => onStartAssessment(topic)}
                                className="px-4 py-2 bg-card border border-border rounded-xl text-sm font-bold hover:border-primary hover:text-primary transition-all shadow-sm flex items-center gap-2 group"
                            >
                                {topic.name}
                                <span className="text-[10px] bg-secondary px-1.5 py-0.5 rounded uppercase">{topic.subject.slice(0, 3)}</span>
                            </button>
                        ))}
                    </div>
                </div>
